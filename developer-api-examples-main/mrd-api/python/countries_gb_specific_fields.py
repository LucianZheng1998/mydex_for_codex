...
MRD_API_ENDPOINT = "https://api-mrd.mydex.org/countries/GB"
...
def request_country(token: str) -> None:
    """
    Call the MRD endpoint for GB with filters: name, idd, maps.
    Prints the JSON response.
    """
    # requests encodes list params as repeated keys: filters=name&filters=idd&filters=maps
    params = {"filters": ["name", "idd", "maps"]}
    headers = {
    ...
    try:
        resp = requests.get(MRD_API_ENDPOINT, headers=headers, params=params, timeout=20)
        resp.raise_for_status()
    except requests.RequestException as e:
