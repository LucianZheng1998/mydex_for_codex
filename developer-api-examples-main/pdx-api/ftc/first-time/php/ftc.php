...
function create_ftc() {
    ...
    $requestData = [
        ...
        // If you know the mydexid already, add it as 'mydexid' parameter here
        ...
        // in the POST FTC callback to your API along with MydexID and member key
    ];
    ...
    $ch = curl_init(MYDEX_PDS_PATH . '/ftc/setup');
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
    curl_setopt($ch, CURLOPT_POST, true);
    curl_setopt($ch, CURLOPT_HTTPHEADER, $headers);
    ...
    if ($httpCode >= 400) {
        throw new Exception("FTC setup failed with status $httpCode: $response");
    }
